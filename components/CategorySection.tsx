  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/95 backdrop-blur-xl">
      ...
      <div className="relative flex flex-col lg:flex-row w-full max-w-6xl max-h-[90vh] bg-slate-900 rounded-3xl overflow-hidden border border-white/10">
        <button onClick={onClose} className="absolute top-4 right-4 z-[120] p-2 bg-black/60 text-white rounded-full"><X /></button>
        <div className="relative w-full lg:flex-1 h-[40vh] bg-black flex items-center justify-center p-4">
          ...
          <button onClick={toggleFavorite} className={`absolute bottom-6 right-6 p-4 rounded-full border transition-all ${isFavorite ? "bg-pink-500 text-white border-pink-400" : "bg-black/50 text-white border-white/20"}`}>
            <Heart className={isFavorite ? "fill-current" : ""} />
          </button>
        </div>
        <div className="w-full lg:w-[400px] p-8 text-left bg-slate-900">
          <h2 className="text-2xl font-black text-white italic uppercase mb-4 tracking-tighter">{image.title}</h2>
          <div className="bg-white/5 p-4 rounded-2xl border border-white/5 mb-6 text-slate-300 text-sm">高品質AIビジュアル素材。商用利用可能。</div>
          <a href={url} download className="flex items-center justify-center gap-2 w-full bg-cyan-500 text-white font-bold py-4 rounded-2xl mb-8">無料ダウンロード</a>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <section className="py-20 px-6 max-w-7xl mx-auto">
      <div className="flex items-end justify-between mb-10 text-left">
        <div><h2 className="text-4xl font-black text-white italic uppercase mb-2 tracking-tighter">{title}</h2><p className="text-slate-400">{description}</p></div>
        <Link href={`/category/${title}`} className="text-cyan-400 font-bold flex items-center gap-2">VIEW ALL <ChevronRight /></Link>
      </div>
