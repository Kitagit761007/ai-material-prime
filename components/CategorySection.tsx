...
const getSeoDescription = (t: string, c: string, id: string) => {
  ...
  const texts = [`「${t}」は${c}をテーマに、最新のAI技術を用いて制作されたデジタルアセットです。プロフェッショナルな制作現場での使用に最適化されています。` ];
  return `${texts[seed % texts.length]} 商用利用可能・クレジット表記不要。`;
};
...
export function DetailModal({ image, url, onClose }: { image: any; url: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    img.src = url;
    img.onload = () => setMetadata(p => ({ ...p, width: img.naturalWidth, height: img.naturalHeight, ratio: getSmartRatio(img.naturalWidth, img.naturalHeight) }));
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/95 backdrop-blur-xl animate-in fade-in duration-300">
      ...
      <div className="relative flex flex-col lg:flex-row w-full max-w-6xl max-h-[95vh] bg-slate-900 rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
        ...
        <div className="w-full lg:w-[400px] p-8 flex flex-col overflow-y-auto bg-slate-900 border-l border-white/5 text-left flex-1">
          <div className="mb-6"><h2 className="text-xl sm:text-2xl font-black text-white italic uppercase tracking-tighter mb-4 leading-tight">{image.title}</h2><div className="bg-white/5 p-4 rounded-2xl border border-white/5 mb-6"><p className="text-slate-300 text-xs leading-relaxed">{getSeoDescription(image.title, image.category, image.id)}</p></div></div>
