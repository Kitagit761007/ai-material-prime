...
const getSeoDescription = (t: string, c: string, id: string) => {
  ...
  return `「${t}」は、${c}をテーマに最新の生成AI技術で描かれた${id}番目の作品です。持続可能な未来へのビジョンを、精緻な質感で表現しました。商用利用可能で、プロフェッショナルな制作に最適化されています。クレジット表記も不要です。`;
};

// 🚀 この DetailModal が他のファイルで使われるため、export が必須です
export function DetailModal({ image, url, onClose }: { image: any; url: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    img.src = url;
    img.onload = () => setMetadata(prev => ({ 
      ...prev, width: img.naturalWidth, height: img.naturalHeight, ratio: getSmartRatio(img.naturalWidth, img.naturalHeight) 
    }));
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/95 backdrop-blur-xl animate-in fade-in">
      ...
      <div className="relative flex flex-col lg:flex-row w-full max-w-6xl max-h-[95vh] bg-slate-900 rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
        <button onClick={onClose} className="absolute top-4 right-4 z-[120] p-2 bg-black/60 hover:bg-black text-white rounded-full border border-white/10 transition-transform active:scale-90"><X className="w-6 h-6" /></button>
        <div className="relative w-full lg:flex-1 h-[40vh] lg:h-auto bg-black flex items-center justify-center p-4 flex-none">
